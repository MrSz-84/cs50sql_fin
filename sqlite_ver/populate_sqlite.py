...
def iter_over_inputs(
    data_set: list[dict[list, str, str]],
    con: sqlite3.Connection,
    cur: sqlite3.Cursor,
    avoid_adding: list[str],
) -> None:
    """
...
def check_for_data_1_field(
    ...
    if len(in_db) == 0:
        ...
        if table_name == "kody_rek":
            ...
            new_data.loc[:, field_name] = new_data.loc[:, ["a", "b"]].apply(
                lambda x: f"{x['a']}@|@{x['b']}", axis=1
            )
...
def add_3_fields(
    data_set: dict[pd.DataFrame, str, list],
    con: sqlite3.Connection,
    cur: sqlite3.Cursor,
) -> None:
    """
...
def get_id_for_submediums(
    fields_: list[str], table_: str, cur: sqlite3.Cursor
    ...
    submedia = df[["Submedium", "Wydawca/Nadawca", "Zasięg medium"]].sort_values(
        by="Submedium"
    )
    submedia.drop_duplicates(
        subset=["Submedium"], keep="first", inplace=True, ignore_index=True
    )
...
def check_for_data_3_fields(
    fields: list[str], table_name: str, submedia: pd.DataFrame, cur: sqlite3.Cursor
) -> tuple[bool, pd.DataFrame]:
    """
    ...
    in_db.rename(
        columns={0: "id", 1: fields[0], 2: fields[1], 3: fields[2]}, inplace=True
    )
...
def get_id_for_ads_desc(
    ...
    spoty.loc[:, "dł. Ujednolicona"] = (
        spoty["dł. Ujednolicona"].map(dl_ujednolicona).astype("Int8")
    )
...
df = pd.read_csv(
    ...
    dtype={
        "Dzień": "category",
        "Dzień tygodnia": "category",
        "Nr. tyg.": "category",
        "Rok": "category",
        "Miesiąc": "category",
        "Zasięg medium": "category",
        "Brand": "object",
        "Produkt(4)": "object",
        "Kod Reklamy": "int32",
        "Opis Reklamy": "object",
        "Typ reklamy": "object",
        "Wydawca/Nadawca": "category",
        "Submedium": "object",
        "dł./mod.": "int8",
        "GG": "int8",
        "MM": "int8",
        ...
        "Koszt [zł]": "Int32",
        "L.emisji": "int8",
        "dł. Ujednolicona": "Int8",
        "Godzina bloku reklamowego": "object",
    },
    encoding="utf-8",
    parse_dates=["Data"],
    low_memory=False,
...
df["Zlepek"] = df.apply(
    lambda x: f"{str(x['Data_str']).strip()+str(x['GG'])+str(x['MM'])+str(x['SS']) \
...
dow2 = ["Poniedziałek", "Wtorek", "Środa", "Czwartek", "Piątek", "Sobota", "Niedziela"]
months = [
    "Styczeń",
    "Luty",
    "Marzec",
...
ad_codes = df.loc[:, ["Kod Reklamy", "Opis Reklamy"]].drop_duplicates(
    subset=["Kod Reklamy"], keep="first", ignore_index=True
)
ad_codes = ad_codes.apply(lambda x: f"{x['Kod Reklamy']}@|@{x['Opis Reklamy']}", axis=1)
typ_rek = df["Typ reklamy"].unique()

data_set = [
    {"data": dow2, "table": "dni_tyg", "field": "dzien_tyg"},
...
Sixteen processing time : {fourteen_diff:.2f}
"""
)
